...
func writeToInflux(lmc LoadMaster) {
  ...
  for _, lm := range lmc.Lm {
    ...
    fmt.Println(fmt.Sprintf("lmsubs,Custid="+strconv.Itoa(lmc.Custid)+",Lmclusterid=1,Name=Xname Substier="+lm.License.Substier+",Subsexpiry="lm.License.Subsexpiry+",Subsexpirydays="lm.License.Subsexpirydays))
